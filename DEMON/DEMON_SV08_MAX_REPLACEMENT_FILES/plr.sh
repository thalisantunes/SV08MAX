...
#if [[ "$commandline" == *"F"* ]]; then
#    adjusted_height=$(echo "$height + 5" | bc)
#else
adjusted_height=$(echo "$height + 4.8" | bc)
#fi
...
# echo "SET_KINEMATIC_POSITION X=$x_value Y=$y_value" >> ${PLR_PATH}/"${plr}"  #for test
# gcode_z="G0 Z$height"

gcode_z="G0 Z$z_value" #for test
gcode_xy="G0 F$f_value X$x_value Y$y_value"  #for test
# gcode_absolute_extrude="$extrude_type"
# gcode_e="G92 E$e_extrude_abs"

echo $gcode_xy >> ${PLR_PATH}/"${plr}" #for test
